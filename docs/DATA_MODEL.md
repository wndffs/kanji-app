...
- `UserSettings`: id, userId, currentCourseId, locale, translationDisplayMode, timezone, dailyLessonLimit, lessonBatchSize, lessonOrderMode, reviewBudget, reviewOrderMode, strictMode, and dashboardWidgets. `currentCourseId` is nullable and resolves only to an active published enrollment at read time.
