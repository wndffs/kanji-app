...
- Postgres: Neon.
- Redis: Upstash Redis. The current API treats Redis as optional, but keep the
  variable ready for background jobs.
...
3. Create an Upstash Redis database and copy its Redis URL.

4. Create the Render API service from `render.yaml`.
   - Set `DATABASE_URL`, `REDIS_URL`, and `WEB_ORIGIN` in the Render dashboard.
...
5. Create the Vercel web project from the same GitHub repository.
   ...
   - `vercel.json` supplies the install, build, and output settings.
